...
class UsersRepository {
  async create({ tenant_id, dni, role, password_hash, active = 1 }, { conn } = {}) {
    const id = generateId();
  ...
  async findByDniWithinTenant(tenant_id, dni, { conn } = {}) {
    const executor = conn || getPool();
    const [rows] = await executor.execute('SELECT * FROM users WHERE tenant_id = ? AND dni = ?', [tenant_id, dni]);
    return rows[0] || null;
  }

  async findById(id, tenant_id, { conn } = {}) {
    const executor = conn || getPool();
    const [rows] = await executor.execute('SELECT * FROM users WHERE id = ? AND tenant_id = ?', [id, tenant_id]);
    return rows[0] || null;
  }

  async update({ id, tenant_id, role, active }, { conn } = {}) {
    const executor = conn || getPool();
    await executor.execute('UPDATE users SET role = ?, active = ? WHERE id = ? AND tenant_id = ?', [role, active, id, tenant_id]);
  }

  async updatePassword({ id, tenant_id, password_hash }, { conn } = {}) {
    const executor = conn || getPool();
